// lib/set-stream.ts (your original file)
"use server";
...
import { bufferedResponse } from "./buffered-response"; // 👈 import this
  ...
  if (await streamContext.hasExistingStream(appId)) {
    return {
      async readableStream() {
        return await streamContext.resumeExistingStream(appId);
      },
      async response() {
...
export async function setStream(
  ...
  const rawBody = stream.toUIMessageStreamResponse().body;
  ...
  const delayedStream = bufferedResponse(rawBody);
  ...
  const resumableStream = await streamContext.createNewResumableStream(appId, () => {
    return delayedStream.pipeThrough(new TextDecoderStream()) as ReadableStream<string>;
  });
